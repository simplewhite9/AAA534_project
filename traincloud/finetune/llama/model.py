...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.cache_k = torch.zeros((args.max_batch_size, args.max_seq_len+args.max_feats, self.n_local_heads, self.head_dim)).cuda()
        self.cache_v = torch.zeros((args.max_batch_size, args.max_seq_len+args.max_feats, self.n_local_heads, self.head_dim)).cuda()
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None, video_feature=None, generate=False):
        ...
        if adapter is not None:
            ...
            if generate:
                extra_mask = torch.zeros(1, 1, seqlen, adapter_len + start_pos).to(mask)
            else:
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, args):
        ...
        self.mc = args.mc # Multiple choice questions
        ...
        self.tau = 100
        self.flag = args.flag
        self.vocab = torch.load('../data/msvd/vocab_token.pth')


    def forward(self, video, text_id, label, vocab_tokens, inference = False, generate=False):
        _bsz, _n_candidates, seqlen = text_id.shape
        ...
        if inference:
            video_feature = video_feature.unsqueeze(1).repeat(1, _n_candidates, 1, 1).view(-1, video_feature.shape[-2], video_feature.shape[-1])
        ...
        if not self.mc and vocab_tokens != '':
            output = output + torch.roll(vocab_tokens, -1, 1)

        output = output[:, :-1, :].reshape(-1, self.vocab_size)

        if inference:
            c_loss = self.inference_criterion(output, label).reshape(_bsz, _n_candidates, -1)
    ...
    def generate(self, text_id, video, start_pos, vocab_tokens, inference = True, generate=False):
        ...
        if not self.mc and vocab_tokens != '':
            output = output + torch.roll(vocab_tokens, -1, 1)[:, start_pos + seqlen - 1,  :] 
        return output.float()