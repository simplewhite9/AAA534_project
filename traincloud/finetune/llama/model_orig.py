...
class Transformer(nn.Module):
    ...
    def forward(self, video, text_id, label, inference=False):
        ...
        for layer in self.layers[-1 * self.adapter_layer:]:
            h = layer(h, start_pos, freqs_cis, mask, adapter[adapter_index].half(), video_feature, self.flag)
    ...
    def generate(self, text_id, video, generate=True):
        ...
        for i, layer in enumerate(self.layers[-1 * self.adapter_layer:]):
            h = layer(h, start_pos, freqs_cis, mask, adapter[i].half(), video_feature, self.flag)
